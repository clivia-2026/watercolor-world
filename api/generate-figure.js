...
Return ONLY raw SVG starting with <svg and ending with </svg>. No explanation.`;
    ...
    // 修复1: glm-4v-flash 传 base64 时，url 字段只需要纯 base64 字符串，不要 data URI 前缀
    // 修复2: 去掉 max_tokens 参数（该参数可能导致"参数有误"错误）
    // 修复3: 如果 glm-4v-flash 仍然报错，可尝试改为 glm-4.6v-flash（新版免费视觉模型）
      ...
      body: JSON.stringify({
        ...
        messages: [{
          ...
          content: [
            {
              type: 'image_url',
              image_url: { url: imageBase64 }   // 只传纯 base64，不加 data:...;base64, 前缀
            },
            {
              type: 'text',
              text: prompt
    ...
    console.log('Zhipu response body:', JSON.stringify(data).slice(0, 500));
