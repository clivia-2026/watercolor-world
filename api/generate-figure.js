...
const SVG_RESPONSE_SCHEMA = {
  type: 'object',
  required: ['subjectType', 'templateHint', 'colors', 'svg'],
  properties: {
...
function sanitizeSvg(svg) {
  ...
  if (!/^<svg[\s\S]*<\/svg>$/.test(clean)) return null;
  if (clean.length > 30000) return null;
  if (!/viewBox=/i.test(clean)) {
    clean = clean.replace('<svg', '<svg viewBox="0 0 220 320"');
  }
  return clean;
}
...
function buildFallbackSvg(analysis = {}) {
  const tpl = TEMPLATES[analysis.templateHint] ? analysis.templateHint : pickFallbackTemplate(analysis);
  const def = DEFAULT_COLORS[tpl];
  const colors = {};
  for (const key of ['hair', 'skin', 'top', 'bottom', 'shoes', 'acc']) {
    colors[key] = normalizeHexColor(analysis.colors?.[key], def[key]);
  }
  return { template: tpl, svg: TEMPLATES[tpl](colors), colors };
}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const svgPrompt = `You are designing a beautiful watercolor illustrated character from an uploaded photo.

Return ONLY a JSON object that matches the requested schema.

Task:
1. Identify the subject from the photo.
2. Pick the closest fallback template hint from:
"girl_walking", "girl_standing", "boy_standing", "person_sitting", "cat", "rabbit", "dog"
3. Extract a muted watercolor palette from the photo.
4. Generate a complete standalone SVG string of a SINGLE full-body figure or pet in an elegant watercolor sketch style.

SVG requirements:
- Transparent background.
- Full character only, centered, bottom-aligned.
- Beautiful, soft, airy, hand-painted watercolor feel.
- Simple face details, clean silhouette, soft pigment blooms, gentle outlines.
- No text, no frame, no scenery, no extra objects.
- Use only safe inline SVG elements and attributes.
- No script, no foreignObject, no external URLs, no CSS files.
- Keep the SVG lightweight and self-contained.
- Use viewBox="0 0 220 320".

Color rules:
- Return exactly 6 hex colors: hair, skin, top, bottom, shoes, acc.
- For animals, hair/bottom/shoes can reuse the fur color.
- Colors should stay muted and tasteful, not neon.

Output quality goal:
- Much prettier and more bespoke than a generic avatar.
- Preserve the subject's vibe, hairstyle/species, and clothing impression from the photo.
- Prioritize charm and watercolor elegance over photorealism.`;
    ...
    const apiRes = await fetch('https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent', {
      ...
      body: JSON.stringify({
        ...
        generationConfig: {
          ...
          responseJsonSchema: SVG_RESPONSE_SCHEMA
        }
      })
    });
    ...
    const safeSvg = sanitizeSvg(analysis?.svg);
    ...
    if (safeSvg) {
      return res.json({
        ...
        templateHint: analysis.templateHint || null
      });
    }

    const fallback = buildFallbackSvg(analysis || {});
    console.log('Fallback template:', fallback.template, 'Colors:', JSON.stringify(fallback.colors));
    return res.json({ svg: fallback.svg, source: 'fallback-template', templateHint: fallback.template });
