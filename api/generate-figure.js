...
function buildHumanEditPrompt() {
  return [
    ...
    '请基于输入人类照片，对主体做高质量水彩风重绘。',
    '整体原则是贴近原照片，只做温和的水彩化处理，不要大改人物本身，不要换人，不要明显重设计。',
    '最重要的是高度保留原照片中的脸和发型。脸型、五官相对位置、眼睛感觉、鼻子嘴巴关系、年龄感、神态、发际线、刘海、发型轮廓、发色必须非常相似。',
    '保留原照片中的发色、发型趋势、刘海感觉、眼镜、帽子、脸部朝向、穿搭轮廓、上衣颜色、下装颜色、包或配饰。',
    '人物的脸和头发不允许明显改动，不允许换脸，不允许改成另一种发型；只把照片转成更干净柔和的水彩插画，不要大幅美颜，不要整容感。',
    ...
    '如果输入照片本身是半身、胸像或头肩构图，请自动补齐下半身、腿部和鞋子，用同风格自然续画，最终输出必须是完整全身人物。',
    '人物必须完整站立在画面中，脚必须出现，不能裁掉腿脚，不能只生成头像或半身。',
    '如果衣服是黑色、深灰、藏蓝等深色，必须保持深色，不要洗成白色、米色或浅灰。',
    '如果额前发型不明显，请自然补成好看的八字刘海或轻碎发，但不要夸张。',
    '整体画风：白底留白很多、干净、高级、轻盈、松弛、精致的时尚水彩插画，笔触简洁，有少量水痕和留白。',
...
function buildAnimalEditPrompt() {
  return [
    ...
    '整体原则是贴近原照片，只做温和的水彩化处理，不要大改动物本身，不要明显重设计。',
    '整体原则是贴近原照片，只做温和的水彩化处理，不要大改物种特征和颜色。',
